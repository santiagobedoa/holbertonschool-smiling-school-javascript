            ...
            i === 0 ? "active" : ""
              ...
              <div class="row">
                  <div class="col-sm-3 text-center">
                      <img class="rounded-circle" src=${$(this)
                        .find("pic_url")
                        .text()} class="d-block w-100" alt="random person image">
                  </div>
                  <div class="col-sm-8 ml-3 d-flex flex-column">
                      <div>&lt;&lt; ${$(this)
                        .find("text")
                        .text()} &gt;&gt;</div>
                      <div class="font-weight-bold mt-3">${$(this)
                        .find("name")
                        .text()}</div>
          ...
          let $html = $(`
                  <div class="text-center col-12 col-sm-6 col-md-3">
                      <div class="carousel-item active">
                          <img class="w-100" src="${$(this)
                            .find("thumb_url")
                            .text()}" alt="smile image">
                          <div class="mx-3">
                              ...
                              <div class="d-flex align-items-center mb-3">
                                  <img src="${$(this)
                                    .find("author_pic_url")
                                    .text()}" class="rounded-circle mr-3 video-carousel-img-profile" alt="profile image">
    ...
    success: function (response) {
      ...
      $(response)
        ...
        .each(function (i, el) {
          ...
          $topics += `<button data-value="${$t}" class="dropdown-item" type="button">${
            $t.substr(0, 1).toUpperCase() + $t.substr(1)
          }</button>`;
        });
            ...
            $("#sort-menu-container")
              .append(`<button class="btn btn-secondary border-0 rounded-0 search-element w-100 text-left" type="button" id="current-sort" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    ${
                      ...
                      $s.substr(1, 3) +
                      " " +
                      $s.substr(5, 1).toUpperCase() +
                      $s.substr(6)
                    }
                  </button>
                  <div class="dropdown-menu" id="sort-menu" aria-labelledby="sort-menu-container"></div>`);
          }
          $sorts += `<button data-value="${$s}" class="dropdown-item" type="button">${
            $s.substr(0, 1).toUpperCase() +
            $s.substr(1, 3) +
            " " +
            $s.substr(5, 1).toUpperCase() +
  ...
  $.displayCourses = function ($q, $t, $s) {
    $.ajax({
      ...
      success: function (response) {
        $("#CoursesLoader").hide();
        $("#coursesvideos").empty();
        let $number = $(response).find("courses").find("course").length;
        $("#numberofvideos").text(
          `${$number === 1 ? "1 video" : $number + " videos"}`
        );
        $(response)
          .find("courses")
          .find("course")
